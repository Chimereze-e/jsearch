REGEX_PATT = {
    ...
    # AWS & Cloud Storage
    # ============================================
    # ✅ ChatGPT's negative lookbehind/lookahead is good, but Python \b is cleaner
    ...
    # ============================================
    # ⚠️ ChatGPT's version is TOO COMPLEX - misses raw Bearer tokens in headers
    # Better: Catch both assignment AND raw header values
    "Authorization": r"(?i)(?:authorization|auth)\s*[:=]\s*['\"]?Bearer\s+[A-Za-z0-9\-._~+/]+=*['\"]?",
    
    # ✅ ChatGPT's context-aware approach is excellent
    "ACCESS_TOKEN": r"(?i)\baccess[_-]?token\b\s*[:=]\s*['\"][A-Za-z0-9._\-]{20,}['\"]",
    
    # ✅ Good improvement on minimum lengths
    "JWT": r"\beyJ[A-Za-z0-9_-]{10,}\.[A-Za-z0-9_-]{20,}\.[A-Za-z0-9_-]{20,}\b",
    
    # ✅ ChatGPT's \n exclusion is smart for avoiding multiline noise
    ...
    # URLs
    # ============================================
    # ✅ ChatGPT's negative lookbehind for @ is brilliant (avoids emails)
    # But require meaningful paths
    ...
    # ⚠️ ChatGPT's version is TOO RESTRICTIVE - requires 3+ depth
    # Better: Require at least one endpoint but allow shallow APIs
